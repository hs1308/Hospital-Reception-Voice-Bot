...
const PulseOrb: React.FC<PulseOrbProps> = ({ state, isEmergency = false }) => {
  const getOrbColor = () => {
    ...
    switch (state) {
      case 'initiating': return 'bg-slate-400 shadow-none animate-pulse';
      ...
      case 'processing': return 'bg-amber-500 shadow-[0_0_60px_-10px_rgba(245,158,11,0.8)] animate-bounce';
      default: return 'bg-slate-300 shadow-none';
    }
  };

  const getStatusLabel = () => {
    if (isEmergency) return 'Emergency Mode';
    switch (state) {
    ...
    <div className="flex flex-col items-center justify-center space-y-12">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-center space-x-1">
          {[1, 2, 3].map(i => (
            <div key={i} className={`w-1.5 h-1.5 rounded-full animate-bounce ${isEmergency ? 'bg-red-400' : 'bg-slate-400'}`} style={{ animationDelay: `${i * 0.15}s` }} />
          ))}
        </div>
      </div>
    </div>
